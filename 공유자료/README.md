...
- [Gap Buffer Data Structure](https://www.geeksforgeeks.org/dsa/gap-buffer-data-structure/) <br>
  갭버퍼에 대한 설명과 구현 코드를 Java, C++ 등으로 확인해볼 수 있어서 좋은 글입니다.
- [Jetpack Compose MeasurePolicy Explained](https://androidengineers.substack.com/p/jetpack-compose-measurepolicy-explained) <br>
  Compose의 MeasurePolicy에 대해 설명하고 이미지를 통해 쉽게 설명해주고 있습니다.
- [You don't understand recomposition](https://kt.academy/article/recomposition#you-dont-understand-recomposition) <br>
  recomposition에 대해 적은 글입니다. 대부분 알고 있는 내용일거라 생각하지만 리마인드 하기에 좋은 글이라 생각됩니다. Composition과 Recomposition의 차이는 뭔가요? 라는 질문에 여러분은 어떻게 답하실건가요? 쉬운 것 같으면서도 대답하기 어렵다면 이 글을 한 번 읽어 보시는 것이 좋다고 생각합니다.
- [runBlocking in practice: Where it should be used and where not](https://kt.academy/article/run_blocking) <br>
  runBlocking이 무엇이고 언제 사용하고, 사용하지 말아야하는지 알려주는 글 입니다.
